...
const FullChatPage = ({ user, friends }) => {
  ...
  // Function to navigate back to dashboard and scroll to top
  const goToDashboard = () => {
    navigate('/');
  ...
  if (width < 700) {
    return (
      <div style={{
        ...
        background:'linear-gradient(135deg, #6366f1 0%, #818cf8 40%, #fbbf24 100%)',
        overflow:'hidden',
        position:'relative',
        ...
        <style>{`
          ...
          .chat-content-scroll { overscroll-behavior: auto; -webkit-overflow-scrolling: touch; }
        `}</style>
        <div style={{
          flex:1,
          display:'flex',
          ...
          {view === 'list' && (
            <div className="chatlist-mobile-anim" style={{
              height:'100%',
              display:'flex',
              flexDirection:'column',
              background:'#fff',
              ...
              <div className="chat-content-scroll" style={{
                ...
                paddingBottom:'80px' // Extra space for Safari
              }}>
                {friends.length === 0 ? (
                  <div style={{color:'#e0e7ff',marginTop:'2em',textAlign:'center'}}>No friends yet.</div>
                ) : friends.map(friend => (
                  <div key={friend.id} onClick={()=>{setSelectedFriend(friend); setView('chat'); navigate(`/chat/${friend.id}`);}} style={{
                    padding:'0.7em 1em',
                    borderRadius:8,
                    marginBottom:6,
                    cursor:'pointer',
                  ...
                  }} tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'){setSelectedFriend(friend); setView('chat'); navigate(`/chat/${friend.id}`);}}}>
                    {friend.username || friend.email}
                  </div>
                ))}
              </div>
              {/* Fixed bottom button for better accessibility */}
              <div style={{
                position:'fixed',
                bottom:0,
                left:0,
          ...
          {view === 'chat' && (
            <div className="chat-mobile-anim">
              <div style={{
                ...
                <button onClick={()=>{setView('list'); setTimeout(()=>navigate('/chat'), 400);}} style={{
                  marginRight:16,
                  background:'none',
                  border:'none',
                  color:'#6366f1',
                  ...
                  outline:'none',
                  boxShadow:'none'
                }} aria-label="Back to chat list">&#8592;</button>
      ...
      <div style={{
        ...
        <div className="chat-content-scroll" style={{
          ...
          {friends.length === 0 ? (
            ...
            <div key={friend.id} onClick={()=>{setSelectedFriend(friend); navigate(`/chat/${friend.id}`);}} style={{
              padding:'0.7em 1em',
              borderRadius:8,
              marginBottom:6,
              cursor:'pointer',
              background:selectedFriend && selectedFriend.id===friend.id?'#eef2ff':'transparent',
              color:selectedFriend && selectedFriend.id===friend.id?'#6366f1':'#222',
              fontWeight:500,
              transition:'background 0.15s,color 0.15s'
            }} tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'){setSelectedFriend(friend); navigate(`/chat/${friend.id}`);}}}>
              {friend.username || friend.email}
            </div>
          ))}
        </div>
        <button onClick={() => {
          navigate('/');
          setTimeout(() => window.scrollTo(0, 0), 100);
        }} style={{margin:'1em',padding:'0.7em 1em',borderRadius:8,background:'none',color:'#6366f1',fontWeight:700,border:'none',cursor:'pointer',boxShadow:'none',outline:'none'}}>Back to Dashboard</button>
      </div>
      {/* Center: Chat Panel */}
      <div style={{
        flex:1,
        display:'flex',
        flexDirection:'column',
        background:selectedFriend?'#2d313a':'#23272f',
        minWidth:0,
        height:'100vh',
        position:'relative',
        overflow:'hidden',
        ...
        <div style={{
          ...
          alignItems:'center',
          padding:'1.2em 1.5em 1em 1.5em',
          borderBottom:selectedFriend?'1px solid #e5e7eb':'none',
          background:selectedFriend?'#fff':'transparent',
          minHeight:70,
          position:'sticky',
          top:0,
          zIndex:2
          ...
          <span style={{
            fontWeight:700,
            fontSize:'1.2em',
            color:selectedFriend?'#6366f1':'#fff',
            textShadow:selectedFriend?'none':'0 2px 8px rgba(0,0,0,0.18)'
          }}>{selectedFriend ? (selectedFriend.username || selectedFriend.email) : 'Select a chat'}</span>
        </div>
        <div style={{
          flex:1,
          display:'flex',
